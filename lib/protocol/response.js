...
const {getSerializationNameById, getSerializationById, getSerializationByName} = require('../serialize');
...
class Response {
    ...
    if (this.status === Response.OK) {
      if (this.data) {
        if (this.data instanceof Error || (this.data.$class && this.data.$class.includes('Exception'))) {
          output.writeByte(RESPONSE_WITH_EXCEPTION, options);
          output.writeObject(this.data.$class ? this.data : {
            $class: 'java.lang.Exception',
            $: this.data,
          }, classMap, 'StringValue', options);
        } else {
          if (this.data) {
            output.writeByte(RESPONSE_VALUE, options);
            output.writeObject(this.data, classMap, this.data.$class ? this.data.$class : 'AnyVal', options);
          } else {
            output.writeByte(RESPONSE_NULL_VALUE, options);
          }
        }
      } else {
      ...
      output.writeUTF(this.errorMsg || 'Exception caught in invocation', options);
    }
  ...
  static decode(buf, options = {}) {
    const [magic_high, magic_low] = buf.slice(0,2).toJSON().data;
    if (magic_high != MAGIC_HIGH || magic_low != MAGIC_LOW) return;
    ...
    if (options.sentReqs)
      options.channel = options.sentReqs.get(packetId);
    const flag = buf[2];
    const sType = flag & SERIALIZATION_MASK;
    const input = getSerializationById(sType).deserialize(buf);
    options.codecType = getSerializationNameById(sType);
    // skip header
    input.skip(16);
    ...
    if (status === Response.OK) {
      if (packetType === 'heartbeat_ack') {
        ...
        const rFlag = input.readObject(options, 'Int32Value');
        switch (parseInt(rFlag, 10)) {
          case RESPONSE_NULL_VALUE:
            break;
          case RESPONSE_VALUE:
            data.appResponse = input.readObject(options);
            break;
          case RESPONSE_WITH_EXCEPTION:
            data.error = input.readObject(options);
            break;
          case RESPONSE_NULL_VALUE_WITH_ATTACHMENTS:
            data.responseProps = input.readObject(options, 'HashMap');
            break;
      ...
      const errorMsg = input.readUTF(options);
      data = {
        appResponse: null,
