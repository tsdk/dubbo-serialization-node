    ...
    if (opts.proto) {
      if (type) {
        ...
        if (Msg) {
          ...
          let data = '';
          try {
            data = Msg.toObject(Msg.decode(buf), { defaults: true, enums: String });
            ...
            data = Msg.toObject(Msg.decodeDelimited(content), { defaults: true, enums: String });
            if (opts.proto[type]) data = data.value;
          }
        ...
        if (methodInfo.responseType) {
          ...
          const buf = decoder.bytes();
          try {
            appResponse = responseType.toObject(responseType.decode(buf), { defaults: true, enums: String });
          } catch (e) {
            const content = Buffer.concat([
              Buffer.from(utils.int2hex(buf.length), 'hex'),
              buf,
            ]);
            appResponse = responseType.toObject(responseType.decodeDelimited(content), { defaults: true, enums: String });
          }
        }
        return appResponse;
      }
